...
```bash
# 1. 把对应架构的产物包拷到服务器
# 2. 解压到 /usr/local（包内路径为 tengine/，解压后得到 /usr/local/tengine）
tar xzf tengine-gm-2.4.1-x86_64-centos7-glibc217.tar.gz -C /usr/local   # x86 服务器
# tar xzf tengine-gm-2.4.1-aarch64-kyv10-glibc228.tar.gz -C /usr/local   # ARM 服务器
